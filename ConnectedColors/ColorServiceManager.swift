...
class ColorServiceManager : NSObject {
    ...
    private let ColorServiceType = "inflight-secret"
    ...
    func sendRequest( intent : String ){
        NSLog("%@", "send Request: \(intent)");
        
        if session.connectedPeers.count > 0 {
            var error : NSError?
            do {
                try self.session.sendData(intent.dataUsingEncoding(NSUTF8StringEncoding, allowLossyConversion: false)!, toPeers: session.connectedPeers, withMode: MCSessionSendDataMode.Reliable)
            } catch var error1 as NSError {
                error = error1
                NSLog("%@", "\(error)")
            }
...
//        self.delegate?.colorChanged(self, colorString: str)
        ...
        if ( str == "water" ||
            ...
            self.delegate?.requestChanged(self, intentString: str);
